...
def esperar_elemento_clickable(driver, selectores, timeout=30):
    ...
    for selector in selectores:
        try:
            ...
            ultimo_error = error
    raise RuntimeError("WhatsApp Web no mostró el campo esperado.") from ultimo_error
...
def crear_driver():
    ...
    driver = webdriver.Chrome(
        service=Service(ChromeDriverManager().install()), options=opciones
    )
...
def abrir_grupo(driver):
    buscador = esperar_elemento_clickable(driver, [
        XPATH_BUSCADOR,  # Selector de la versión usada por los scripts originales.
    ...
    resultados = driver.find_elements(By.XPATH, f'//span[@title="{GRUPO_NOMBRE}"]')
    if resultados:
        resultados[0].click()
    else:
        # Es el método de los scripts originales 1, 2 y 3; sirve cuando el
        # nombre no está expuesto en el atributo title por una nueva versión.
...
def caja_mensaje(driver):
    return esperar_elemento_clickable(driver, [
        XPATH_MENSAJE,  # Selector de la versión usada por los scripts originales.
...
def enviar_texto(driver, texto):
    ...
    # WhatsApp envía de forma asíncrona; no cerrar Chrome justo tras Enter.
    time.sleep(1)
    ...
    try:
        boton_continuar = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, '//a[contains(@href,"web.whatsapp.com/send")]')
            )
        )
        boton_continuar.click()
        time.sleep(5)
    except Exception:
        pass  # El botón no aparece cuando WhatsApp abre el chat directamente.
    enviar_texto(driver, texto)
    # Los chats abiertos por URL necesitan un margen adicional para que el
    # mensaje salga de la cola antes de que el finally cierre el navegador.
    time.sleep(4)
...
def ejecutar_automatizacion(accion):
    ...
    try:
        ...
        print(f"Error durante la automatización: {error}")
    finally:
        if driver:
            driver.quit()
...
def enviar_mensaje_2():
    ...
    ejecutar_automatizacion(lambda driver: (abrir_grupo(driver), enviar_texto(
        driver, "Recordatorio Automático: Inicio del horario de descanso, No olviden marcar."
    )))
...
def enviar_mensaje_3():
    ...
    ejecutar_automatizacion(lambda driver: (abrir_grupo(driver), enviar_texto(
        driver, "Recordatorio Automático: Fin de horario de descanso, No olviden marcar."
    )))
...
def enviar_mensaje_4():
    ...
    ejecutar_automatizacion(lambda driver: (abrir_grupo(driver), enviar_texto(
        driver, "Recordatorio Automático: Hora de limpiar y ordenar el taller, por favor todos los presentes"
    )))
...
def enviar_mensaje_5():
    ...
    def accion(driver):
        abrir_grupo(driver)
        enviar_mencion(driver, "Recordatorio Automático: Realizar mantenimiento preventivo a los *Grupos Generadores*, asignado a ", "Angel")
        time.sleep(1)
        enviar_directo(driver, "595994246602", "Recordatorio Automático: Realizar mantenimiento preventivo a los *Grupos Generadores*, asignado a Angel Lezcano.")
...
def enviar_mensaje_6():
    ...
    def accion(driver):
        abrir_grupo(driver)
        enviar_mencion(driver, "Recordatorio Automático: Realizar registro de los *horómetros de equipos de Maquinado*, asignado a ", "Ronald")
        time.sleep(1)
        enviar_directo(driver, "595982090279", "Recordatorio Automático: Realizar registro de los *horómetros de equipos de Maquinado*, asignado a Ronald Caceres.")
